...
function typology_load_js() {
	...
	if ( typology_get_option( 'minify_js' ) ) {
		...
		wp_enqueue_script( 'typology-main', get_parent_theme_file_uri( '/assets/js/min.js' ), array( 'jquery' ), TYPOLOGY_THEME_VERSION, true );
		...
		foreach ( $scripts as $id => $script ) {
			wp_enqueue_script( 'typology-'.$id, get_parent_theme_file_uri( '/assets/js/'. $script ), array( 'jquery' ), TYPOLOGY_THEME_VERSION, true );
		}
	}
