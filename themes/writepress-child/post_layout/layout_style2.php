...
if( count( $note ) != 0 ) { ?>
<div class="note">
	<p><i><?php echo get_post_meta($post->ID, 'note', true) ?></i></p>
</div>
<?php 
} else { 
// do nothing; 
}?>	
<div class="preptricks">
	<?php 
	$peoplenbr = get_post_meta($post->ID, 'Nombre de personnes', false);
	if( count( $peoplenbr ) == 1 ) { ?>
	<div class="peoplenbr">
		<i class="fa fa-users" aria-hidden="true"></i> Pour <b><?php echo get_post_meta($post->ID, 'Nombre de personnes', true) ?></b> personnes
	</div>
	<?php
	...
	<?php 
	$preptime = get_post_meta($post->ID, 'Temps de préparation', false);
	if( count( $preptime ) == 1 ) { ?>
	<div class="preptime">
		<i class="fa fa-clock-o" aria-hidden="true"></i> <b>Temps de préparation:</b>
		<?php echo get_post_meta($post->ID, 'Temps de préparation', true) ?>
	</div>
	<?php
...
// do nothing; 
           ...
           <?php if( ( $social_sharing_box == 'on' && $show_hide_sharing != 'no' ) ||  ( $social_sharing_box == 'off' && $show_hide_sharing == 'yes' ) ): ?>
            <div class="share-box">
            <h4>Partagez cette article!</h4>
            <?php //writepress_social_sharing(); ?><!-- Go to www.addthis.com/dashboard to customize your tools --> <div class="addthis_inline_share_toolbox"></div>
          </div>
            <?php endif; ?>
          <?php //Share Box End ?>
