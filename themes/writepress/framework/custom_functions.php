...
if( ! function_exists( 'writepress_current_page_title_bar' ) ) {
	...
	//TitleBar Show/hide
		...
		if( is_search() ) {
			if($page_title_bar == 'on'){					
					$title = __('Search results for:', 'writepress') . get_search_query();
			}
		  ...
		  <div class="pagetitle_parallax" data-parallax="<?php if($titlebar_parallax_bg == 'No'){echo '';}else if($titlebar_parallax_bg == 'Yes'){echo esc_url($image_url);}else{ if($page_title_bg_parallax == 'on'){echo esc_url($image_url); } } ?>" style="background:<?php echo esc_attr($bg_color);?> url(<?php echo esc_url($image_url);?>);">
			<div class="pagetitle_parallax_content">
			  <div class="zolo-container">
				...
				<?php if (function_exists('writepress_breadcrumbs')) writepress_breadcrumbs(); ?>
				<?php endif; ?>
			  </div>
			</div>
	...
	function writepress_single_page_nav() {
		...
		if ( ! $next && ! $previous )
			...
			if($post_navigation_style == 'style1'){
				...
				previous_post_link( '%link', __( '<span class="meta-nav post-meta-nav"><span class="meta-nav">&larr;</span> Previous</span> <span class="post-meta-nav-title">%title</span>', 'writepress' ) );
				
				
				
				
				next_post_link( '%link', _x('<span class="meta-nav post-meta-nav">Next <i class="fa fa-long-arrow-right" aria-hidden="true"></i></span><span class="post-meta-nav-title">%title</span>', 'Next post link', 'writepress' ) );
				echo '</div>';
